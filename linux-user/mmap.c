...
abi_long target_madvise(abi_ulong start, abi_ulong len, int flags)
{
    if (!guest_range_valid_untagged(start, len)) {
        return -TARGET_EINVAL;
    }

    if ((flags & MADV_DONTNEED) == 0)
        return 0;

    mmap_lock();
    abi_long ret = get_errno(madvise(g2h_untagged(start), len, MADV_DONTNEED));
    page_remove_tags(start, start + len);
    mmap_unlock();
    return ret;
}
